...
async fn async_main(args: Vec<String>) {
  ...
  // Run UI on other thread forever, copying state in.
  let ui_t = {
    let headless_display_name = headless_display_name.clone();
  ...
  loop {
    ...
    // Anything to do here? Ensure the GUI is fullscreen + on connected_projector_ws?
  }
